...
FRAME_ID_SUSPICIOUS_GAP_THRESHOLD = MAX_UINT16_FRAME_ID // 2
...
class SelkiesStreamingApp:
    ...
    def send_ws_cursor_data(self, data):
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(_broadcast_cursor_helper(), self.async_event_loop)
        else:
    ...
    def set_framerate(self, framerate):
        ...
        logger_gst_app.info(f'Framerate for {self.encoder} set to {self.framerate}. Restart pipeline if active.')
...
async def get_new_res(res_str):
    ...
    try:
        process = await subprocess.create_subprocess_exec('xrandr', stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    if not screen_name:
        logger_gst_app_resize.warning('Could not determine connected screen from xrandr.')
        return curr_res, new_res, resolutions, max_res_str, screen_name
...
async def resize_display(res_str):  # e.g., res_str is "2560x1280"
    ...
    if not screen_name:
        logger_gst_app_resize.error('Cannot resize display via xrandr, no screen identified.')
        return False
    ...
    if res_str not in available_resolutions:
        ...
        try:
            ...
            logger_gst_app_resize.error(f'Failed to generate modeline for {res_str}: {e}')
            return False
        ...
        add_mode_proc = await subprocess.create_subprocess_exec(
            *cmd_add, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        if add_mode_proc.returncode != 0:
            ...
            delmode_proc = await subprocess.create_subprocess_exec(
                'xrandr', '--delmode', screen_name, res_str, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            await delmode_proc.communicate()

            rmmode_proc = await subprocess.create_subprocess_exec(
                'xrandr', '--rmmode', res_str, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            await rmmmode_proc.communicate()
            return False
        logger_gst_app_resize.info(f'Successfully ran: {" ".join(cmd_add)}')

    logger_gst_app_resize.info(f"Applying xrandr mode '{target_mode_to_set}' for screen '{screen_name}'.")
    cmd_output = ['xrandr', '--output', screen_name, '--mode', target_mode_to_set]
    set_mode_proc = await subprocess.create_subprocess_exec(*cmd_output, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    logger_gst_app_resize.info(f"Successfully applied xrandr mode '{target_mode_to_set}'.")
    return True
...
async def generate_xrandr_gtf_modeline(res_wh_str):
    ...
    try:
        ...
        try:
            ...
            logger_gst_app_resize.warning('cvt command failed or not found, trying gtf.')
        ...
        raise Exception(f'Failed to generate modeline using {tool_name} for {res_wh_str}: {e}') from e
    except ValueError:
        raise Exception(f'Invalid resolution format for modeline generation: {res_wh_str}')
    match = re.search(r'Modeline\s+"([^"]+)"\s+(.*)', modeline_output)
    if not match:
        raise Exception(f'Could not parse modeline from {tool_name} output: {modeline_output}')
...
def parse_dri_node_to_index(node_path: str) -> int:
    ...
    if not node_path or not node_path.startswith('/dev/dri/renderD'):
        if node_path:
            logger.warning(
                f"Invalid DRI node format: '{node_path}'. Expected '/dev/dri/renderD...'. VA-API will be disabled."
            )
        return -1
    try:
...
async def _run_xrdb(dpi_value, logger):
    ...
    try:
        ...
        process = await subprocess.create_subprocess_exec(*cmd_xrdb, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = await process.communicate()
        ...
        if xrdb_success:
            ...
            logger.warning(
                f'Failed to load {xresources_path_str} using xrdb. RC: {process.returncode}, Error: {stderr.decode().strip()}'
            )
        ...
        if not which('pgrep') or not which('kill'):
            ...
            pgrep_proc = await subprocess.create_subprocess_exec(
                'pgrep', 'xsettingsd', stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            if pgrep_proc.returncode == 0:
                ...
                if pid_output:
                    ...
                    kill_proc = await subprocess.create_subprocess_exec(
                        'kill', '-1', pid, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                    )
                    ...
                    if kill_proc.returncode == 0:
                        ...
                        logger.warning(
                            f'Failed to send SIGHUP to xsettingsd process {pid}. Error: {kill_stderr.decode().strip()}'
                        )
            else:
...
async def _get_xfce_session_env(logger):
    ...
    try:
        proc_pid = await subprocess.create_subprocess_exec(
            'pgrep', '-o', '-x', 'xfce4-session', stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
async def _run_xfconf(dpi_value, logger):
    ...
    async def run_command(cmd, success_msg, failure_msg):
        try:
            process = await subprocess.create_subprocess_exec(
                *cmd, env=session_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    ...
    if not await run_command(
        cmd_cursor, f'Successfully set cursor size to {cursor_size}', 'Failed to set cursor size using xfconf-query'
    ):
        return False
...
async def _run_mate_gsettings(dpi_value, logger):
    ...
    try:
        ...
        result_mate_window_scale = await subprocess.create_subprocess_exec(
            *cmd_gsettings_mate_window_scale, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        stdout_mate_window, stderr_mate_window = await result_mate_window_scale.communicate()
        if result_mate_window_scale.returncode == 0:
            logger.info(
                f'Successfully set MATE window-scaling-factor to {mate_window_scaling_factor} (for DPI {dpi_value}) using gsettings.'
            )
            ...
            if 'No such schema' in stderr_text or 'No such key' in stderr_text:
                logger.debug(
                    f"gsettings: Schema/key 'org.mate.interface window-scaling-factor' not found. Error: {stderr_text}"
                )
            else:
                logger.warning(
                    f'Failed to set MATE window-scaling-factor using gsettings. RC: {result_mate_window_scale.returncode}, Error: {stderr_text}'
                )
    except Exception as e:
    ...
    try:
        ...
        result_mate_font_dpi = await subprocess.create_subprocess_exec(
            *cmd_gsettings_mate_font_dpi, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        if result_mate_font_dpi.returncode == 0:
            ...
            if 'No such schema' in stderr_font_text or 'No such key' in stderr_font_text:
                logger.debug(
                    f"gsettings: Schema/key 'org.mate.font-rendering dpi' not found. Error: {stderr_font_text}"
                )
            else:
                logger.warning(
                    f'Failed to set MATE font-rendering DPI using gsettings. RC: {result_mate_font_dpi.returncode}, Error: {stderr_font_text}'
                )
    except Exception as e:
...
async def set_dpi(dpi_setting):
    ...
    if which('startplasma-x11'):
        ...
        logger_gst_app_resize.info(
            f'No specific DE session binary found (KDE, XFCE, MATE, i3, Openbox). Attempting generic xrdb as a fallback for DPI {dpi_value}.'
        )
        if await _run_xrdb(dpi_value, logger_gst_app_resize):
            any_method_succeeded = True
    ...
    if not any_method_succeeded:
        logger_gst_app_resize.warning(
            f'No DPI setting method succeeded for DPI {dpi_value} (Attempted for: {de_name_for_log}).'
        )
    ...
    if which('gsettings'):
        try:
            ...
            process_set = await subprocess.create_subprocess_exec(
                *cmd_set, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            logger_gst_app_resize.warning(f'Error trying to set GNOME cursor size via gsettings: {e}')
...
class DataStreamingServer:
    ...
    async def broadcast_display_config(self):
        ...
        payload = {'type': 'display_config_update', 'displays': connected_displays}
    ...
    def _pcmflux_audio_callback(self, result_ptr, user_data):
        ...
        if self.is_pcmflux_capturing and result_ptr and self.pcmflux_audio_queue is not None:
            ...
            if result.data and result.size > 0:
                data_bytes = bytes(ctypes.cast(result.data, ctypes.POINTER(ctypes.c_ubyte * result.size)).contents)
        ...
        if self.pcmflux_module:
            try:
                if self.pcmflux_capture_loop:
                    await self.pcmflux_capture_loop.run_in_executor(None, self.pcmflux_module.stop_capture)
            except Exception as e:
    ...
    async def shutdown_pipelines(self):
        ...
        if self.display_clients:
            stop_bp_tasks = [
                self._ensure_backpressure_task_is_stopped(disp_id) for disp_id in self.display_clients.keys()
            ]
            ...
            while True:
                ...
                if time_since_last_ack > STALLED_CLIENT_TIMEOUT_SECONDS:
                    if display_state.get('backpressure_enabled', True):
                        data_logger.warning(
                            f"Client stall for '{display_id}': No ACK in {time_since_last_ack:.1f}s. Forcing backpressure."
                        )
                    display_state['backpressure_enabled'] = False
                elif effective_desync_frames > allowed_desync_frames:
                    if display_state.get('backpressure_enabled', True):
                        data_logger.warning(
                            f"Backpressure TRIGGERED for '{display_id}'. S:{server_id}, C:{client_id} (EffDesync:{effective_desync_frames:.1f}f > Allowed:{allowed_desync_frames:.1f}f)."
                        )
                    display_state['backpressure_enabled'] = False
                else:
                    if not display_state.get('backpressure_enabled', True):
                        data_logger.info(
                            f"Backpressure LIFTED for '{display_id}'. S:{server_id}, C:{client_id} (EffDesync:{effective_desync_frames:.1f}f <= Allowed:{allowed_desync_frames:.1f}f)."
                        )
                    display_state['backpressure_enabled'] = True
    ...
    def _parse_settings_payload(self, payload_str: str) -> dict:
        ...
        parsed['is_manual_resolution_mode'] = get_bool('is_manual_resolution_mode')
        parsed['manual_width'] = get_int('manual_width')
        parsed['manual_height'] = get_int('manual_height')
        parsed['audio_bitrate'] = get_int('audio_bitrate')
        parsed['initialClientWidth'] = get_int('initialClientWidth')
        parsed['initialClientHeight'] = get_int('initialClientHeight')
        parsed['jpeg_quality'] = get_int('jpeg_quality')
        parsed['paint_over_jpeg_quality'] = get_int('paint_over_jpeg_quality')
    ...
    async def _apply_client_settings(
        ...
        data_logger.info(f"Applying and sanitizing client settings for '{display_id}' (initial={is_initial_settings})")
        ...
        async with self._reconfigure_lock:
            ...
            if server_is_manual:
                data_logger.info(
                    f"Server override is active. Forcing manual resolution from server configuration for display '{display_id}'."
                )
                try:
                    ...
                    data_logger.error(
                        f'Server override failed: Could not parse manual resolution from server config. Error: {e}. Falling back.'
                    )
                    target_w = 1024
                    target_h = 768
            ...
            display_state['h264_streaming_mode'] = sanitize_value(
                'h264_streaming_mode', settings.get('h264_streaming_mode')
            )
            display_state['jpeg_quality'] = sanitize_value('jpeg_quality', settings.get('jpeg_quality'))
            display_state['paint_over_jpeg_quality'] = sanitize_value(
                'paint_over_jpeg_quality', settings.get('paint_over_jpeg_quality')
            )
            display_state['use_paint_over_quality'] = sanitize_value(
                'use_paint_over_quality', settings.get('use_paint_over_quality')
            )
            display_state['h264_paintover_crf'] = sanitize_value(
                'h264_paintover_crf', settings.get('h264_paintover_crf')
            )
            display_state['h264_paintover_burst_frames'] = sanitize_value(
                'h264_paintover_burst_frames', settings.get('h264_paintover_burst_frames')
            )
            ...
            if self.input_handler:
                self.enable_binary_clipboard = sanitize_value(
                    'enable_binary_clipboard', settings.get('enable_binary_clipboard')
                )
            ...
            if new_dpi is not None and new_dpi != old_settings.get('scaling_dpi'):
                data_logger.info(
                    f'DPI changed from {old_settings.get("scaling_dpi")} to {new_dpi}. Applying system-level change.'
                )
            ...
            video_params_changed = any(display_state.get(key) != old_settings.get(key) for key in video_params_list)
    ...
    async def ws_handler(self, websocket):
        if self.is_secure_mode:
            ...
            data_logger.info(
                f'Client {websocket.remote_address} authenticated with token. Role: {permissions.get("role")}, Slot: {permissions.get("slot")}'
            )
        ...
        self._system_monitor_task_ws = asyncio.create_task(_collect_system_stats_ws(self._shared_stats_ws))
        if GPUtil.getGPUs():
            self._gpu_monitor_task_ws = asyncio.create_task(
                _collect_gpu_stats_ws(self._shared_stats_ws, gpu_id=gpu_id_for_stats)
            )
        self._stats_sender_task_ws = asyncio.create_task(_send_stats_periodically_ws(websocket, self._shared_stats_ws))
        self._network_monitor_task_ws = asyncio.create_task(_collect_network_stats_ws(self._shared_stats_ws, self))
        ...
        try:
                ...
                if isinstance(message, bytes):
                    ...
                    if msg_type == 0x01:
                        if (
                            ...
                            try:
                                active_uploads_by_path_conn[active_upload_target_path_conn].write(payload)
                            except Exception as e_write:
                                data_logger.error(f'File write error for {active_upload_target_path_conn}: {e_write}')
                                try:
                        ...
                        if not PULSEAUDIO_AVAILABLE:
                            if len(payload) > 0:
                                data_logger.warning('PulseAudio library not available. Skipping microphone data.')
                            continue
                        if pulse is None:
                            if len(payload) > 0:
                                data_logger.warning('PulseAudio client not connected. Skipping microphone data.')
                            continue

                        if not mic_setup_done:
                            data_logger.info('Performing PulseAudio virtual microphone setup check...')
                            try:
                                    ...
                                    if self.is_pcmflux_capturing:
                                        try:
                                            ...
                                            for output in source_outputs:
                                                if (
                                                    hasattr(output, 'proplist')
                                                    and output.proplist.get('application.name') == 'pcmflux'
                                                ):
                                                ...
                                                if connected_source and connected_source.name != self.audio_device_name:
                                                    ...
                                                    for source in current_source_list:
                                                        if source.name == self.audio_device_name:
                                                            ...
                                                            break
                                                    if correct_source:
                                                        pulse.source_output_move(
                                                            pcmflux_output.index, correct_source.index
                                                        )
                                                        data_logger.info(
                                                    ...
                                                    data_logger.info(
                                                        f"pcmflux correctly connected to '{connected_source.name}'"
                                                    )
                                            else:
                        ...
                        if not mic_setup_done or not payload:
                            if not mic_setup_done and len(payload) > 0:
                                data_logger.warning('Mic setup not complete, skipping mic data.')
                            continue
                    ...
                    if perms and perms.get('role') == 'viewer':
                        ...
                        if active_mk_token and perms.get('token') == active_mk_token:
                            allowed_viewer_prefixes.extend([
                                'kd',
                                'ku',
                                'kr',
                                'm',
                        ...
                        if not any(message.startswith(prefix) for prefix in allowed_viewer_prefixes):
                            data_logger.warning(
                                f'DENIED unauthorized message from viewer {websocket.remote_address}: {message[:100]}...'
                            )
                            continue
                    ...
                    if message.startswith('FILE_UPLOAD_START:'):
                        if 'upload' not in settings.file_transfers:
                            data_logger.warning(
                                'Client tried to upload a file, but uploads are disabled by server settings.'
                            )
                            continue
                        ...
                        try:
                            ...
                            if (
                                not path_components
                                or sane_rel_path.startswith(os.sep)
                                or sane_rel_path.startswith('/')
                                or sane_rel_path.startswith('\\')
                                or '..' in path_components
                            ):
                                data_logger.error(
                                    f"Invalid or malicious relative path from client: '{rel_path_from_client}'. Discarding."
                                )
                                continue
                            ...
                            if not intended_parent_dir_abs.startswith(real_upload_dir_abs):
                                data_logger.error(
                                    f"Path escape attempt detected: '{final_server_path}' (from client: '{rel_path_from_client}') is outside of '{real_upload_dir_abs}'. Discarding."
                                )
                                continue
                            ...
                            if target_dir and target_dir != real_upload_dir_abs and not os.path.exists(target_dir):
                                if not os.path.abspath(target_dir).startswith(real_upload_dir_abs):
                                    data_logger.error(
                                        f"Directory creation escape attempt: '{target_dir}' is outside of '{real_upload_dir_abs}'. Discarding."
                                    )
                                    continue
                                try:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    data_logger.warning(
                                        f'Error closing previous upload stream {active_upload_target_path_conn}: {e_close_old}'
                                    )
                                del active_uploads_by_path_conn[active_upload_target_path_conn]
                            ...
                            data_logger.error(f'Invalid FILE_UPLOAD_START format: {message}')
                        except Exception as e_fup_start:
                            data_logger.error(f'FILE_UPLOAD_START processing error: {e_fup_start}', exc_info=True)
                        ...
                        if (
                            ...
                            active_uploads_by_path_conn[active_upload_target_path_conn].close()
                            data_logger.info(f'Upload finished: {active_upload_target_path_conn}')
                            del active_uploads_by_path_conn[active_upload_target_path_conn]
                        active_upload_target_path_conn = None
                        ...
                        try:
                            ...
                            if client_role == 'viewer':
                                data_logger.info(
                                    f'Viewer client {websocket.remote_address} sent initial SETTINGS. Syncing with current stream state.'
                                )
                            ...
                            if display_id in ['primary', 'display2']:
                                ...
                                if existing_client_info:
                                    ...
                                    if (
                                        old_ws
                                        and old_ws is not websocket
                                        and old_ws.state == websockets.protocol.State.OPEN
                                    ):
                            ...
                            if display_id not in self.display_clients:
                                ...
                                data_logger.info(
                                    f"Client is taking over existing display '{display_id}'. Updating state for new connection."
                                )
                                display_state = self.display_clients[display_id]
                            ...
                            await self._apply_client_settings(
                                websocket, parsed_settings, not initial_settings_processed, client_role
                            )
                            if not initial_settings_processed:
                                initial_settings_processed = True
                                ...
                                if not video_is_active:
                                    data_logger.error(
                                        f'FATAL: Initial reconfiguration completed, but video pipeline did not start.'
                                    )
                                async with self._reconfigure_lock:
                                    audio_is_active = self.is_pcmflux_capturing
                                    if not audio_is_active and PCMFLUX_AVAILABLE and display_id == 'primary':
                                        data_logger.info(
                                            'Initial setup: Primary client connected, audio not active, attempting start.'
                                        )
                                        await self._start_pcmflux_pipeline()
                                    elif not PCMFLUX_AVAILABLE and not audio_is_active:
                                        data_logger.warning(
                                            'Initial setup: Audio pipeline (server-to-client) cannot be started (pcmflux not available).'
                                        )

                        except json.JSONDecodeError:
                            data_logger.error(f'SETTINGS JSON decode error: {message}')
                        except Exception as e_set:
                            data_logger.error(f'Error processing SETTINGS: {e_set}', exc_info=True)
                        ...
                        if perms and perms.get('role') == 'viewer':
                            ...
                            if now - last_req_time < 30.0:
                                data_logger.warning(
                                    f'Throttled START_VIDEO request from viewer {websocket.remote_address}. Ignoring.'
                                )
                                continue
                        ...
                        if client_display_id and client_display_id in self.display_clients:
                            ...
                            if hasattr(self, 'display_layouts') and client_display_id in self.display_layouts:
                                ...
                                data_logger.info(
                                    f"Found existing layout for '{client_display_id}'. Starting capture with: {layout}"
                                )
                                try:
                                    await self._start_capture_for_display(
                                    ...
                                    data_logger.error(
                                        f"Failed to restart individual stream for '{client_display_id}': {e}",
                                        exc_info=True,
                                    )
                                    await self.reconfigure_displays()
                            else:
                                data_logger.warning(
                                    f"No layout found for '{client_display_id}' on START_VIDEO. Performing full reconfiguration."
                                )
                                await self.reconfigure_displays()
                                await websocket.send('VIDEO_STARTED')
                        else:
                            data_logger.info(
                                f'Received START_VIDEO from a shared client ({websocket.remote_address}). Triggering reconfiguration.'
                            )
                            await self.reconfigure_displays()
                        ...
                        async with self._reconfigure_lock:
                            ...
                            if PCMFLUX_AVAILABLE:
                                ...
                                data_logger.warning(
                                    'START_AUDIO: Cannot start server-to-client audio (pcmflux not available).'
                                )
                        ...
                        try:
                            ...
                            if self.capture_cursor != new_capture_cursor:
                                ...
                                data_logger.info(
                                    f'SET_NATIVE_CURSOR_RENDERING: Value {new_capture_cursor} is already set.'
                                )
                        ...
                        try:
                            ...
                            if IS_WAYLAND and client_display_id:
                                if which('kwin_wayland'):
                                    ...
                                    data_logger.info(
                                        f'KDE Wayland detected. Restarting stream with scale {scale_val} for {client_display_id}'
                                    )
                                    await self._stop_capture_for_display(client_display_id)
                                    if hasattr(self, 'display_layouts') and client_display_id in self.display_layouts:
                                        layout = self.display_layouts[client_display_id]
                            ...
                            if CURSOR_SIZE > 0:
                                ...
                                data_logger.warning(
                                    'CURSOR_SIZE is not positive. Skipping cursor size adjustment based on DPI.'
                                )
                        ...
                        if not settings.command_enabled[0]:
                            data_logger.warning(
                                "Received 'cmd' message, but command execution is disabled by server settings."
                            )
                            continue
                        ...
                        if len(toks) > 1:
                            ...
                            try:
                                ...
                                data_logger.info(
                                    f"Successfully launched command: '{command_to_run}' with PID {process.pid}"
                                )
                            except Exception as e:
                        ...
                        if message.startswith('js,') and self.is_secure_mode:
                            ...
                            if not perms or not perms.get('token'):
                                data_logger.warning(
                                    f'BLOCK (Secure Mode): Gamepad input from {websocket.remote_address} dropped. Client has no token/perms.'
                                )
                                continue
                            ...
                            if server_slot is None:
                                data_logger.warning(
                                    f'BLOCK (Secure Mode): Gamepad input from {websocket.remote_address} dropped. Client token has no assigned slot.'
                                )
                                continue
                            try:
                                ...
                                if (int(server_slot) - 1) != client_index:
                                    data_logger.warning(
                                        f'BLOCK (Secure Mode): Gamepad input from {websocket.remote_address} dropped. Client sent for index {client_index}, but is assigned slot {server_slot}.'
                                    )
                                    continue
                            except (IndexError, ValueError):
                                data_logger.warning(
                                    f'BLOCK (Secure Mode): Malformed gamepad message from {websocket.remote_address}: {message}'
                                )
                                continue

                        if self.is_secure_mode and message.split(',')[0] in [
                            'kd',
                            'ku',
                            'm',
                            'm2',
                        ...
                        if self.input_handler and hasattr(self.input_handler, 'on_message'):
            ...
            data_logger.error(f'Error in Data WS handler for {raddr}: {e_main_loop}', exc_info=True)
        finally:
            ...
            if disconnected_display_id:
                ...
                data_logger.info(
                    f"Client for '{disconnected_display_id}' disconnected. Removing and triggering full display reconfiguration."
                )
                await self.reconfigure_displays()
            else:
            ...
            if self._frame_backpressure_task and not self._frame_backpressure_task.done():
                if not self.clients:
                    data_logger.info(f'Last client ({raddr}) disconnected. Cancelling frame backpressure task.')
                else:
                    data_logger.info(
            ...
            if (
                ...
                and locals()['active_upload_target_path_conn'] in locals()['active_uploads_by_path_conn']
            ):
            ...
            if not self.clients:
                data_logger.info(
                    f'Last client ({raddr}) disconnected. All pipelines should have been stopped by reconfigure_displays.'
                )
                self.capture_cursor = False
                async with self._reconfigure_lock:
        ...
        while not self.stop_server.done():
            ...
            try:
                async with ws_async.serve(
                    ...
                    wait_closed_task = asyncio.create_task(_current_server_instance.wait_closed())
                    ...
                    if self.stop_server in done:
                        ...
                        break
                    data_logger.warning(f'Data WS Server on port {self.port} stopped unexpectedly. Restarting.')
            except OSError as e:
                data_logger.error(f'OSError starting Data WS on port {self.port}: {e}. Retrying in 5s...')
                await asyncio.sleep(5)
            except asyncio.CancelledError:
                data_logger.info(f'Data WS run_server task for port {self.port} cancelled.')
                break
            except Exception as e:
                data_logger.error(
    ...
    async def stop(self):
        ...
        if self.server:
            ...
            try:
                ...
                data_logger.warning(f'Timeout closing Data WS listener on port {self.port}.')
    ...
    async def _cleanup_client(self, websocket, display_id):
        ...
        if self._is_reconfiguring:
            data_logger.warning(
                f"Client '{display_id}' disconnected DURING a reconfiguration. "
    ...
    async def _run_detached_command(self, cmd_list: list, description: str):
        ...
        try:
            proc = await asyncio.create_subprocess_shell(
                shell_command, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL
            )
        except Exception as e:
    ...
    async def _run_command(self, cmd, description):
        ...
        try:
            ...
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
                data_logger.error(f'Failed ({description}). RC: {proc.returncode}, Stderr: {stderr.decode().strip()}')
                return False
            return True
        except Exception as e:
    ...
    async def _get_current_monitors(self):
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                'xrandr', '--listmonitors', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await proc.communicate()
            if proc.returncode == 0:
    ...
    async def reconfigure_displays(self):
        ...
        async with self._reconfigure_lock:
            ...
            try:
                ...
                if (
                    not IS_WAYLAND
                    and current_display_count > 1
                    and self._wm_swap_is_supported
                    and not self._is_wm_swapped
                ):
                    ...
                    try:
                        ...
                        data_logger.error(
                            f'Could not write Openbox config to {config_path}: {e}. Proceeding without custom config.'
                        )
                ...
                if self.capture_instances or any(d.get('backpressure_task') for d in self.display_clients.values()):
                    ...
                    stop_capture_tasks = [
                        ...
                        for inst in self.capture_instances.values()
                        if inst.get('module')
                    ]
                    ...
                    if screen_name:
                        ...
                        for monitor_name in current_monitors:
                            await self._run_command(
                                ['xrandr', '--delmonitor', monitor_name], f'cleanup monitor {monitor_name}'
                            )
                    return
                ...
                if not IS_WAYLAND:
                    ...
                    for monitor_name in current_monitors:
                        await self._run_command(
                            ['xrandr', '--delmonitor', monitor_name], f'delete old monitor {monitor_name}'
                        )
                    ...
                    if total_mode_str not in available_resolutions:
                        ...
                        try:
                            ...
                            await self._run_command(
                                ['xrandr', '--newmode', total_mode_str] + modeline_params.split(), 'create new mode'
                            )
                            await self._run_command(
                                ['xrandr', '--addmode', screen_name, total_mode_str], 'add new mode'
                            )
                        except Exception as e:
                            data_logger.error(f'FATAL: Could not create extended mode {total_mode_str}: {e}. Aborting.')
                            return
                    await self._run_command(
                        ['xrandr', '--fb', total_mode_str, '--output', screen_name, '--mode', total_mode_str],
                        'set framebuffer',
                    )
                    ...
                    if 'primary' in layouts:
                        await self._run_command(['xrandr', '--output', screen_name, '--primary'], 'set primary output')
                    ...
                    if client_data and client_data.get('video_active', False):
                        ...
                        try:
                            ...
                            data_logger.error(
                                ...
                                f'This display will not stream. Error: {e}',
                                exc_info=False,
                            )
                    else:
        ...
        try:
            while True:
                ...
                if display_id == 'primary':
                    ...
                    if (
                        not client_info
                        or not client_info.get('ws')
                        or not client_info.get('backpressure_enabled', True)
                    ):
                        queue.task_done()
                        continue
    ...
    async def _start_capture_for_display(self, display_id: str, width: int, height: int, x_offset: int, y_offset: int):
        ...
        data_logger.info(
            f"Preparing to start capture for display='{display_id}': Res={width}x{height}, Offset={x_offset}x{y_offset}"
        )
        ...
        try:
            ...
            await self.capture_loop.run_in_executor(
                None, capture_module.start_capture, settings, StripeCallback(queue_data_for_display)
            )

            self.capture_instances[display_id] = {'module': capture_module, 'sender_task': sender_task}
    ...
    def _get_capture_settings(self, display_id, width, height, x, y):
        ...
        if encoder == 'jpeg':
            ...
            cs.paint_over_jpeg_quality = display_state.get(
                'paint_over_jpeg_quality', self._initial_paint_over_jpeg_quality
            )
        else:  # H.264 modes
            cs.output_mode = 1
            ...
            cs.h264_paintover_burst_frames = display_state.get(
                'h264_paintover_burst_frames', self._initial_h264_paintover_burst_frames
            )
...
async def _collect_system_stats_ws(shared_data, interval_seconds=1):
    data_logger.debug(f'System monitor loop (WS mode) started, interval: {interval_seconds}s')
    try:
        while True:
...
async def _collect_gpu_stats_ws(shared_data, gpu_id=0, interval_seconds=1):
    data_logger.debug(f'GPU monitor loop (WS mode) for GPU {gpu_id}, interval: {interval_seconds}s')
    try:
        gpus = GPUtil.getGPUs()
        if not gpus:
        ...
        while True:
            try:
                ...
                data_logger.error(f'GPU monitor (WS): Error getting stats for ID {gpu_id}: {e_gpu_stat}')
...
async def _collect_network_stats_ws(shared_data, server_instance, interval_seconds=2):
    ...
    data_logger.debug(f'Network monitor loop (WS mode) started, interval: {interval_seconds}s')
    try:
        while True:
            await asyncio.sleep(interval_seconds)
            current_time = time.monotonic()
    ...
    try:
        ...
        if data_server_instance and display_id in data_server_instance.display_clients:
            ...
            if client_info.get('width') == target_w and client_info.get('height') == target_h:
                logger_gst_app_resize.info(
                    f'Redundant resize request for {display_id} to {target_w}x{target_h}. No action.'
                )
                return
            ...
            if IS_WAYLAND:
                logger_gst_app_resize.info(
                    f'Wayland Resize: Updating {display_id} to {target_w}x{target_h} and restarting pipeline.'
                )
                ...
                logger_gst_app_resize.info(
                    f"Display client '{display_id}' dimensions updated to {target_w}x{target_h}. Triggering reconfiguration."
                )
...
async def ws_entrypoint():
    ...
    async def handle_tokens_post(request):
        ...
        try:
            ...
            if not isinstance(new_token_data, dict):
    ...
    if 'DEV_MODE' in os.environ:
        try:
            dev_version_file = pathlib.Path('../../addons/gst-web-core/selkies-version.txt')
    ...
    logger.info(f'Initial Encoder: {initial_encoder}, Framerate: {TARGET_FRAMERATE}')
    ...
    data_server.input_handler = input_handler
    ...
    if ENABLE_RESIZE:
        ...
        input_handler.on_scaling_ratio = lambda scale_val: logger.warning('Scaling disabled.')
    ...
    if hasattr(input_handler, 'connect'):
        tasks_to_run.append(asyncio.create_task(input_handler.connect(), name='InputConnect'))
    ...
    if hasattr(input_handler, 'start_cursor_monitor'):
        tasks_to_run.append(asyncio.create_task(input_handler.start_cursor_monitor(), name='CursorMon'))
    ...
    try:
        ...
        all_tasks_for_cleanup = [t for t in tasks_to_run if t and t is not data_server_task and not t.done()]
        ...
        if input_handler:
            ...
            if hasattr(input_handler, 'stop_js_server') and asyncio.iscoroutinefunction(input_handler.stop_js_server):
                await input_handler.stop_js_server()
            if hasattr(input_handler, 'disconnect') and asyncio.iscoroutinefunction(input_handler.disconnect):
                await input_handler.disconnect()
        if data_server and hasattr(data_server, 'stop') and asyncio.iscoroutinefunction(data_server.stop):
